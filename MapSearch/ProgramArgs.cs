...
public class ProgramArgs
{
    ...
    [ArgDescription("Radius in miles"), ArgRange(1, 30), ArgShortcut("-r")]
    public int Radius { get; set; } = 30;

    [ArgRequired(IfNot = "Type"), ArgCantBeCombinedWith("Type"), ArgDescription("Search term"), ArgShortcut("-s")]
    public string SearchTerm { get; set; }

    [ArgRequired(IfNot = "SearchTerm"), ArgCantBeCombinedWith("SearchTerm"), ArgDescription("Comma seperated list of types in quotes"),
     ArgShortcut("-t")]
    ...
    public async Task Main()
    {
        try
        {
            var results = await GoogleMapsSearch.SearchCabinetShopsAsync(GoogleApiKey, Zips,
                GoogleMapsSearch.MilesToMeters(Radius).ToString(CultureInfo.InvariantCulture), SearchTerm, Type);
            results = RemoveDuplicates(results);
            results = results.OrderBy(r => r.SearchZip).ThenBy(r => r.Name).ToList(); // Sort by name
            foreach (var result in results)
            {
                Console.WriteLine(
                    $"Name: {result.Name}, Vicinity: {result.Address}, Phone: {result.PhoneNumber}, Website: {result.Website}");
            }
            ...
            {
                await csv.WriteRecordsAsync(results.Select(s => new
                    { s.Name, s.Address, s.PhoneNumber, s.Website, s.SearchZip }));
            }
